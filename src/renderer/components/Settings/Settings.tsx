...
export default function Settings({ onClose }: SettingsProps) {
  ...
  const [xtreamPortals, setXtreamPortals] = useState<{ url: string; user: string; pass: string }[]>([])
  ...
  // Load Xtream portals on mount
  useEffect(() => {
    window.api.xtream.getPortals().then(setXtreamPortals).catch(() => {})
  }, [])

  const handleAddPortal = useCallback(async () => {
    if (!newPortalUrl || !newPortalUser || !newPortalPass) return
    const updated = await window.api.xtream.addPortal(newPortalUrl, newPortalUser, newPortalPass)
    setXtreamPortals(updated)
    setNewPortalUrl('')
    setNewPortalUser('')
    setNewPortalPass('')
    // Force-refresh IPTV M3U cache so new portal channels appear immediately
    window.api.iptvM3u.getAllSources(true).catch(() => {})
  }, [newPortalUrl, newPortalUser, newPortalPass])

  const handleRemovePortal = useCallback(async (portal: { url: string; user: string; pass: string }) => {
  ...
  const handleImportFile = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Accept both raw portal array and { portals, ... } wrapper from iptvgen
      let raw: { url: string; username: string; password: string }[] = []
      if (Array.isArray(data)) {
        raw = data
      } else if (data.portals && Array.isArray(data.portals)) {
        raw = data.portals
      }
      // Map scraper field names (username/password) to ours (user/pass)
      const mapped = raw.map(p => ({
        url: p.url,
        user: p.username || (p as any).user,
        pass: p.password || (p as any).pass,
      })).filter(p => p.url && p.user && p.pass)
      if (mapped.length === 0) return
      const result = await window.api.xtream.importPortals(mapped)
      setImportResult(result.added)
      setXtreamPortals(await window.api.xtream.getPortals())
      window.api.iptvM3u.getAllSources(true).catch(() => {})
    } catch {
      // ignore bad JSON
  ...
  <div className={styles.settingGroup}>
    ...
    <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', marginBottom: 8 }}>
      <input
        type="text"
        className={styles.input}
        placeholder="Portal URL (e.g. http://example.com:8080)"
        value={newPortalUrl}
      ...
      <button
        ...
        disabled={!newPortalUrl || !newPortalUser || !newPortalPass}
      >
        Add Portal
      </button>
    </div>
    ...
    <div style={{ marginBottom: 8 }}>
      ...
      {importResult !== null && (
        <span style={{ marginLeft: 8, color: '#4ade80', fontSize: 13 }}>
          Imported {importResult} portal(s)
        </span>
      )}
    </div>
      </>
    ...
    {xtreamPortals.length === 0 ? (
      <div style={{ color: 'rgba(255,255,255,0.3)', fontSize: 13, padding: '4px 0' }}>
        No portals saved yet.
      </div>
    ) : (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        {xtreamPortals.map((p) => (
          <div
            key={`${p.url}|${p.user}|${p.pass}`}
            style={{
              display: 'flex',
              gap: 8,
              alignItems: 'center',
          ...
          >
            <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              <strong>{p.url}</strong> — {p.user}
            </span>
            <button
              tabIndex={0}
              className={styles.connectBtn}
              style={{ background: '#ef4444', padding: '2px 10px', fontSize: 12 }}
